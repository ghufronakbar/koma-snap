...
function sampleLuminance(source: Uint8ClampedArray, width: number, height: number, column: number, row: number, multiplier: number) {
...
function comicChannel(channel: number, luminance: number, shadowLift: number) {
    const saturated = clamp(luminance + (channel - luminance) * 1.55 + shadowLift, 0, 255);
    return clamp(Math.round(saturated / 51) * 51, 25, 247);
}
export function filterRows(request: FilterRequest, start: number, end: number) {
